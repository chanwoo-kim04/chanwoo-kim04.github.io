...
After simplified, we can expect that the sum can be evaluated by applying a known identity, which is typicall derived from the derivative of a finite geometric series. 
...
3. *Evaluate the Series Using a Known Identity*
    ...
    Our targent series is $\sum j \cdot r^j$. 
    This is simply $r$ times the derivative we just found:
    
    $$
    \begin{aligned}
      \sum_{j=1}^n j \cdott r^j &= r \sum_{j=1}^n j \cdot r^{j-1} = r \cdot \dv{G}{r} \\
