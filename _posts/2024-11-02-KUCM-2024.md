...
**Solution Strategy:** 
The strategy is to leverage the $\rank \qty(B) = 1$ condition by representing $B$ as an outer product, $uv^{\top}$. 
This transforms the problem into analyzing $\det \qty(A \pm uv^{\top})$, which is a standard "rank-$1$ update" structure. 
...
Since $\rank \qty(B) = 1$, it can be written as the outer product of two column vectors, $u$ and $v$: $B = uv^{\top}$.
