...
3. *Express the Quadratic Form in the Eigenbasis:*
    ...
    Represent the real vector $\vec{v}$ as a linear combination of the eigenvectors: $\vec{v} = \sum_{k=1}^n c_k \vec{v}_k$. 
    Because the eigenvectors are orthogonal, the quadratic form simplifies to $S = \sum_{k=1}^n \abs{c_k}^2 \lambda_k$. 
...
1. *Set Up Parametrization and Area Formulas:*
    ...
    Let the rod's midpoint be $M = \qty(x \qty(t), y \qty(t))$ and its angle with the $x$-axis be $\theta$. 
    The endpoints are then:
    
    $$
    \begin{aligned}
      P &= \qty(x_P, y_P) = \qty(x + \frac{1}{2} \cos \theta, y + \frac{1}{2} \sin \theta) \\
      Q &= \qty(x_Q, y_Q) = \qty(x - \frac{1}{2} \cos \theta, y - \frac{1}{2} \sin \theta) 
    \end{aligned}
    $$
...
3. *Substitute and Simplify the Combined Integral:*
    ...
    The differentials are $\dd x_P = \ddx - \frac{1}{2} \sin t \dd t$, $\dd y_P = \dd y + \frac{1}{2}\cos t \dd t$, and similarly for $Q$. 
