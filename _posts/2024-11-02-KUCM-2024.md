...
**Solution Strategy:** 
This is a constrained optimization problem where the objective function and constraints exhibit a clear cyclic symmetryc. 
This strong symmetry suggest that a linear algebra approach will be more natural than direct calculus methods, as [Lagrange Multiplier](https://en.wikipedia.org/wiki/Lagrange_multiplier). The strategy is to reframe the problem in a vector space by representing the sequence $\qty(a_1, \dots, a_n)$ as a vector $\vec{v} \in \mathbb{R}^n$. 
...
The optimization of a quadratic form is fundamentall connected to the eigenvalues of its associated operator. 
Therefore, the plan is to find the eigenvalues of the cyclic shift operator. 
The maximum values of $S$ will correspond to the largest eigenvalue, but we must consider the constraint of orthgonality to the vector of all ones. This constraint will likely exclude the eigenvector associated with one of the eigenvalues, leaving the largest remaining eigenvalue as the maximum value of the expression.
